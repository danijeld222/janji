...
namespace Core 
{
	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					COREERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Logger::GetCoreLogger())
				{
					COREERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
	...
	private:
		...
		// NOTE - Dani - You must already own lock on m_Mutex before calling InternalEndSession()
...
#define CORE_PROFILE 0
	...
	#define CORE_PROFILE_SCOPE(name) constexpr auto fixedName = ::Core::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
